...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: GETAPIURL(end_point + "/GetAssignRequestGrid"),
        type: "Get",
            ...
            if (response != null) {
                ...
                $("#data_table").dataTable({
                    ...
                    columns: [
                        ...
                        { data: 'assetName' },
                        { data: 'requestType' },
                        { data: 'queryAssignedBy' },
                        { data: 'status' },
                        { data: 'requestDate','render': function (data, type, full, meta) {return moment(data).format('DD - MMMM - YYYY')}},
                        { data: 'AssignedDate','render': function (data, type, full, meta) {return moment(data).format('DD - MMMM - YYYY')}},
                    ],
                    "order": [[0, "asc"]],
                    "pageLength": 10,
                });
...
function ckvalidation() {
    ...
    if (txt_comtype.val() == '') {
        ck = 1;
        _Error = 'Please Select Complain Type ';
        txt_asset.focus();
    }
