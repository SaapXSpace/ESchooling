...
function ckvalidation() {
    ...
    if (txt_type.val() == '') {
        ck = 1;
        _Error = 'Request Type is Required';
        txt_type.focus();
    }
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: GETAPIURL(end_point + "/GetRequestType"),
        type: "Get",
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddRequestType",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateRequestType",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetRequestTypeById", _id,_type, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    var _Id = data['id'];
    var _type = data['type'];
    DELETE(end_point + "/DeleteRequestType",_Id,_type,function () {
        Onload();
    })
});
