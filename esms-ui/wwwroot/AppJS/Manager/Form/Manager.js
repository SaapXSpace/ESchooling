import  {GETAPIURL,FILLCOMBO,FILLCOMBOUSER, GETBYID,POST,PUT,DELETE,CLEAR}  from "../Service/ApiService.js";
...
function discon(){
    AssignRequestOnload();Onload(); CLEAR(); btn_update.hide();btn_save.show()
}
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: GETAPIURL("/ManagerLovService/GetRequestsGrid"),
        type: "Get",
        ...
        success: function (response) {
            ...
            var action_button = ' ';
            action_button += "<a href='#' class='btn-assign fas fa-user' data-toggle='tooltip' title='Assigne'></a> ";
...
function AssignRequestOnload() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            action_button += " <a href='#' class='btn-delete fas  fa-times' data-toggle='tooltip' title='Cencel Request'></a> ";
            if (response != null) {
                $('#data_table_assign').DataTable().clear().destroy();
                $("#data_table_assign").dataTable({
                    data: response.data,
                    destroy: true,
                    retrieve: true,
                    columns: [
                        ...
                        { data: 'assetName' },
                        { data: 'requestUser' },
                        { data: 'requestDate','render': function (data, type, full, meta) {return moment(data).format('DD - MMMM - YYYY HH:mm:ss')}},
                        { data: 'approved','render': function (data, type, full, meta) {if (data) {return '✔'; }else { return '✘'; }  }},
                        { data: 'userName' },
                        { data: 'date','render': function (data, type, full, meta) {return moment(data).format('DD - MMMM - YYYY HH:mm:ss')}},
                        { data: 'status' }
                        //{ data: 'active','render': function (data, type, full, meta) {if (data) {return '✔'; }else { return '✘'; }  }},
                    ],
                    "order": [[0, "asc"]],
...
function ckvalidation() {
    ...
    if (!ck_approved[0].checked) {
        ck = 1;
        _Error = 'Please Approve First ';
        ck_approved.focus();
    }
...
$('div').on('click', '#openmodal', function (e) {
    ...
    CLEAR();btn_update.hide();btn_save.show()
    $('#txt_request').val(0).trigger('change');
    $('#txt_crew').val(0).trigger('change');
    var current_date = moment(new Date()).format('YYYY-MM-DD')
    $('#txt_date').val(current_date); 
    $('#data_Model').modal();  
...
card_total.addEventListener("click", function() {
    var searchBox;
    var isTab1Active = (document.getElementById("tab-1").className).includes('active')
    if (isTab1Active) {
        searchBox = document.getElementById("data_table_filter").getElementsByTagName("input")[0];
    }else{
        searchBox = document.getElementById("data_table_assign_filter").getElementsByTagName("input")[0];
    }
    searchBox.value = ""
