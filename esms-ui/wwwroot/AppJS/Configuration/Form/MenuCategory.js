...
function ckvalidation() {
    ...
    if (txt_name.val() == '') {
        ck = 1;
        _Error = 'Menu Module Name is required';
        txt_name.focus();
    }
    if (txt_icon.val() == '') {
        ck = 1;
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: GETAPIURL(end_point + "/GetMenuCategory"),
        type: "Get",
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddMenuCategory",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateMenuCategory",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetMenuCategoryById", _id,_name, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    DELETE(end_point + "/DeleteMenuCategory",_Id,_name,function () {
        Onload();
    })
});
