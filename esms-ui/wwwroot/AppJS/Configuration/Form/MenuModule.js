...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: GETAPIURL(end_point + "/GetMenuModule"),
        type: "Get",
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddMenuModule",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateMenuModule",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetMenuModuleById", _id,_name, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    DELETE(end_point + "/DeleteMenuModule",_Id,_name,function () {
        Onload();
    })
});
