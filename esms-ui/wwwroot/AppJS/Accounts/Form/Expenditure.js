...
function discon(){
    ...
    $('#sel_company').val(-1).trigger('change');
    $('#sel_acchead').val(-1).trigger('change');
    $('#txt_payer').val(localStorage.getItem("UserName"));
...
function LoadCompany() {
    var $element = $('#sel_company').select2(); 
    FILLCOMBO('/api/v1/ConfigurationLovService/GetCompanyLov',$element,"Company")
}

// LOAD ACCOUNT HEAD LOV
function LoadAccountHead() {
    var $element = $('#sel_acchead').select2(); 
    FILLCOMBO('/api/v1/AccountsLovService/GetAccountHeadLov',$element,"Account Head")
}
...
function petchdata(response){
    ...
    $('#txt_payer').val(response.payer);
    $('#txt_recipient').val(response.recipient);
    $('#sel_transa').val(response.transaction).trigger('change');
    $('#txt_totalAmount').val(response.amount);
    ...
    $('#sel_company').val(response.companyId).trigger('change');
    $('#sel_acchead').val(response.headId).trigger('change');
    setTimeout(() => {
...
function ckvalidation() {
    ...
    var txt_payer = $('#txt_payer');   
    var txt_recipient = $('#txt_recipient');   
    var sel_transa = $('#sel_transa');   
    ...
    var sel_company = $('#sel_company');   
    var sel_acchead = $('#sel_acchead');  

    if (sel_paytype.val() == 'O') {
        
        if (sel_account.val() == -1 || sel_account.val()==0) {
            ck = 1;
            _Error = 'Select Account because you choose to transaction online';
            sel_account.focus();
        }
        if (sel_accounttype.val() == -1 || sel_accounttype.val()==0) {
            ck = 1;
            _Error = 'Select Account Type for Account because you choose to transaction online';
    ...
    // if (txt_narr.val() == '') {
    //     ck = 1;
    //     _Error = 'Enter Purpose For Transaction';
    //     txt_narr.focus();
    // }
    ...
    if (txt_recipient.val() == '') {
        ck = 1;
        _Error = 'Select Recipient Name, its required';
        txt_recipient.focus();
    }

    // if (txt_payer.val() == '') {
    //     ck = 1;
    //     _Error = 'Select Payer Name, its required';
    //     txt_payer.focus();
    // }

    if (sel_acchead.val() === -1 || sel_acchead.val() === 0 || sel_acchead.val() === "-1") {
        ck = 1;
        _Error = 'Select Account Head';
        sel_acchead.focus();
    }

    if (sel_company.val() === -1 || sel_company.val() === 0 || sel_company.val() === "-1") {
        ck = 1;
        _Error = 'Select Company Name';
        sel_company.focus();
    }
...
function Onload() {
    ...
    $.ajax({
        url: GETAPIURL(end_point + "/GetLedger"),
        type: "Get",
        ...
        success: function (response) {
            ...
            if (response != null) {
                ...
                if (response.permissions.print_permission) {
                    voucher_button += " <a href='#' class='btn-expvoucher fas fa-fw fa-receipt' data-toggle='tooltip' style='color:#03588C' title='Receipt()'></a> ";
                }
                if (response.data != null) {
                    ...
                    var datatablesButtons = $("#data_table").DataTable({
                        ...
                        columns: [
                            ...
                            { data: 'payer' },
                            { data: 'recipient' },
                            { data: 'transaction','render': function (data, type, full, meta) {if (data == "D") {return 'DEBIT'; } else { return 'CREDIT'; }  }},
                            { data: 'amount' },
...
// PRINT BUTTON EVENT
$('table').on('click', '.btn-expvoucher', function (e) {
    e.preventDefault();
    var currentRow = $(this).closest("tr");
    var data = $('#data_table').DataTable().row(currentRow).data();
    var _id = data['id'];
    //location.href = "../Report/ExpenceVoucher?Id="+btoa(_id);
    var encodedId = btoa(_id);
    var url = "../Report/ExpenceVoucher?Id=" + encodedId;
...
$('div').on('click', '#openmodal', function (e) {
    ...
    $('#sel_company').val(-1).trigger('change');
    $('#sel_acchead').val(-1).trigger('change');
    $('#txt_payer').val(atob(localStorage.getItem("UserName")));
});
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddLedger",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateLedger",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetLedgerById", _id,_name, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    DELETE(end_point + "/DeleteLedger",_Id,_name,function () {
        Onload();
    })
});
