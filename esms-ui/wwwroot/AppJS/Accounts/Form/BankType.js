...
function ckvalidation() {
    ...
    if (txt_name.val() == '') {
        ck = 1;
        _Error = 'BankType Name is required';
        txt_name.focus();
    }
...
function Onload() {
    ...
    $.ajax({
        url: GETAPIURL(end_point + "/GetBankType"),
        type: "Get",
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddBankType",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateBankType",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetBankTypeById", _id,_name, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    DELETE(end_point + "/DeleteBankType",_Id,_name,function () {
        Onload();
    })
});
