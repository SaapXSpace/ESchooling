...
$(document).ready(function () {  
    end_point = '/api/v1/CourseCategory';
    discon();
  });
...
function ckvalidation() {
    ...
    if (txt_name.val() == '') {
        ck = 1;
        _Error = 'CourseCategory Name is required';
        txt_name.focus();
    }
...
function Onload() {
    ...
    $.ajax({
        url: GETAPIURL(end_point + "/GetCourseCategory"),
        type: "Get",
...
$('form').on('click', '#btn_sav', function (e) {
    ...
    POST(end_point + "/AddCourseCategory",_cre,function () {
        discon();
    });
});
...
$('form').on('click', '#btn_upd', function (e) {
    ...
    if (ckval == 1) { return; }
    var _cre = ck.creteria;
    PUT(end_point + "/UpdateCourseCategory",_cre,function () {
        discon();
    });
});
...
$('table').on('click', '.btn-edit', async function (e) { //Edit Start
    ...
    await GETBYID(end_point + "/GetCourseCategoryById", _id,_name, function (response) {
        petchdata(response)
    })
...
$('table').on('click', '.btn-delete', function (e) {
    ...
    DELETE(end_point + "/DeleteCourseCategory",_Id,_name,function () {
        Onload();
    })
});
