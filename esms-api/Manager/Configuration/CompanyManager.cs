...
namespace API.Manager.Payroll.Setup
{
    public class CompanyManager : IManager
    {
        ...
        public async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try {
                var _Table = await _context.Companies.Where (a =>  a.Action != Enums.Operations.D.ToString ()).OrderBy (o => o.Name).ToListAsync ();
        ...
        public async Task<ApiResponse> AddAsync(object model,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                string error = "";
                bool _Code = _context.Companies.Any(rec => rec.Code.Trim().ToLower().Equals(_model.ShortName.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                bool _ShortNameExists = _context.Companies.Any(rec => rec.ShortName.Trim().ToLower().Equals(_model.ShortName.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                bool _NameExists = _context.Companies.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                bool _NtnExists = _context.Companies.Any(rec => rec.NTN.Equals(_model.NTN) && rec.NTN != string.Empty && rec.Action != Enums.Operations.D.ToString());
                bool _StnExists = _context.Companies.Any(rec => rec.STN.Equals(_model.STN) && rec.NTN != string.Empty && rec.Action != Enums.Operations.D.ToString());
                ...
                if (_ShortNameExists || _NameExists || _StnExists || _NtnExists)
                {
                    apiResponse.statusCode = StatusCodes.Status409Conflict.ToString();
        ...
        public async Task<ApiResponse> UpdateAsync(object model,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                string error = "";
               bool _ShortNameExists = _context.Companies.Any(rec => rec.ShortName.Trim().ToLower().Equals(_model.ShortName.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                bool _Code = _context.Companies.Any(rec => rec.Code.Trim().ToLower().Equals(_model.Code.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                bool _NameExists = _context.Companies.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                bool _NtnExists = _context.Companies.Any(rec => rec.NTN.Equals(_model.NTN) && rec.NTN != string.Empty && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                bool _StnExists = _context.Companies.Any(rec => rec.STN.Equals(_model.STN) && rec.NTN != string.Empty && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                ...
                var result = _context.Companies.Where (a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    apiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    apiResponse.message = "Record not found ";
                    return apiResponse;
                ...
                result.LogoImage = _model.LogoImage;
                result.NTN = _model.NTN;
                result.STN = _model.STN;
        ...
        public async Task<ApiResponse> DeleteAsync(Guid _Id,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                var _UserId = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserId.ToString())?.Value.ToString();
                var result = _context.Companies.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    apiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    apiResponse.message = "Record not found ";
                    return apiResponse;
