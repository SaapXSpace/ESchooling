...
{
    public class MenuModuleManager : IManager
    {
        private readonly AppDBContext _context;
        public MenuModuleManager(AppDBContext context)
        {
            _context = context;
        }
        ...
        public async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try {
                var _Table = await _context.MenuModules.Include(x => x.Companies).Where (a =>  a.Action != Enums.Operations.D.ToString ()).OrderBy (o => o.Name).ToListAsync ();
        ...
        public async Task<ApiResponse> GetDataByIdAsync(Guid _Id,ClaimsPrincipal _User)
        {
            var apiResponse = new ApiResponse ();
            try {
                var _Table = await _context.MenuModules.Include(x => x.Companies).Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
        ...
        public async Task<ApiResponse> AddAsync(object model,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                var _model = (MenuModule) model;
                string error = "";
                bool _NameExists = _context.MenuModules.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                bool _Code = _context.MenuModules.Any(rec => rec.Code.Trim().ToLower().Equals(_model.Code.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                ...
                await _context.MenuModules.AddAsync (_model);
        ...
        public async Task<ApiResponse> UpdateAsync(object model,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                string error = "";
                bool _NameExists = _context.MenuModules.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                bool _Code = _context.MenuModules.Any(rec => rec.Code.Trim().ToLower().Equals(_model.Code.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                ...
                var result = _context.MenuModules.Where (a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    apiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    apiResponse.message = "Record not found ";
                    return apiResponse;
        ...
        public async Task<ApiResponse> DeleteAsync(Guid _Id,ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                var _UserId = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserId.ToString())?.Value.ToString();
                var result = _context.MenuModules.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    apiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    apiResponse.message = "Record not found ";
                    return apiResponse;
