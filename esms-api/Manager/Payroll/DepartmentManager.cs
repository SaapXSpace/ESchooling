    ...
    public class DepartmentManager : IManager
    {
        ...
        public async Task<ApiResponse> UpdateAsync(object model,  ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                var _model = (Department) model;
                string error = "";
                bool _Code = _context.Departments.Any (rec => rec.Code.Trim ().ToLower ().Equals (_model.Code.Trim ().ToLower ()) && rec.Id !=_model.Id && rec.Action != Enums.Operations.D.ToString ());
        ...
        public async Task<ApiResponse> DeleteAsync(Guid _Id,  ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                var _UserId = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserId.ToString())?.Value.ToString();
                var result = _context.Departments.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    apiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    apiResponse.message = "Record not found ";
                    return apiResponse;
